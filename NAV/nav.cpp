...
Nav::Nav(Account *online_account, QWidget *parent) :
    QNav(parent), ui(new Ui::Nav), goal_sec(0), online_account(online_account), campus(Cp_Shahe), velocity(0), cross(false),
    ...
    findpath_begin(-2), findpath_end(-2), search_end(-2), ant_spot(-2), is_anting(false), before_v(0)
{
    ...
    shahe.load("/mySoftware/C++Qt/myProject/NAV/images/shahe");
    benbu.load("/mySoftware/C++Qt/myProject/NAV/images/benbu");
    between.load("/mySoftware/C++Qt/myProject/NAV/images/between");
...
//    view->setAlignment(Qt::AlignLeft|Qt::AlignTop);
    ...
    connect(pbt_cancel, &QPushButton::clicked, this, &Nav::cancel);
    ...
    connect(pbt_findpath_dis, &QPushButton::clicked, this, &Nav::choose_findpath_dis);
    connect(pbt_findpath_time, &QPushButton::clicked, this, &Nav::choose_findpath_time);
    ...
    connect(pbt_antfind_next, &QPushButton::clicked, this, &Nav::antfindNext);
    connect(pbt_antfind_start, &QPushButton::clicked, this, &Nav::antfindStart);
    connect(pbt_antfind_dis, &QPushButton::clicked, this, &Nav::antfindStartDis);
    connect(pbt_antfind_time, &QPushButton::clicked, this, &Nav::antfindStartTime);

    //模糊搜索的标签和按钮
    lb_fuzzysearch = new QLabel;
    pbt_fuzzysearch_reenter = new QPushButton("重新输入");
    connect(pbt_fuzzysearch_reenter, &QPushButton::clicked, this, &Nav::reenter);
...
void Nav::mouseDoubleClickEvent (QMouseEvent *e) {
    int x = e->x()-offset_x;
    int y = e->y()-offset_y;
...
bool Nav::eventFilter(QObject *watched, QEvent *event)
{
    //对于findpath的按钮
...
void Nav::initAction() {
    findpath = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/search"),"搜索路径", this);
    findpath->setStatusTip ("搜索路线");
    connect(findpath, &QAction::triggered, this, &Nav::findPath);

    search_spot = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/search"),"搜索地点", this);
    search_spot->setStatusTip ("搜索地点");
    connect(search_spot, &QAction::triggered, this, &Nav::searchSpot);

    search_near = new QAction(QIcon("：/mySoftware/C++Qt/myProject/NAV/images/search"),"", this);
    search_near->setStatusTip ("");
    connect(search_near, &QAction::triggered, this, &Nav::searchNear);

    ant_find = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/search"),"多点寻路", this);
    ant_find->setStatusTip ("多点寻路");
    connect(ant_find, &QAction::triggered, this, &Nav::antFind);

    switch_shahe = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/switch_shahe"),"切换到沙河", this);
    switch_shahe->setStatusTip ("切换到沙河");
    connect(switch_shahe, &QAction::triggered, this, &Nav::switchShahe);

    switch_between = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/switch_between"),"切换到校外", this);
    switch_between->setStatusTip ("切换到校外");
    connect(switch_between, &QAction::triggered, this, &Nav::switchBetween);

    switch_benbu = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/switch_benbu"),"切换到本部", this);
    switch_benbu->setStatusTip ("切换到本部");
    connect(switch_benbu, &QAction::triggered, this, &Nav::switchBenbu);

    set_time = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/set_time"),"设置时间", this);
    set_time->setStatusTip ("设置时间");
    connect(set_time, &QAction::triggered, this, &Nav::setTime);

    log_search = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/log"),"日志搜索", this);
    log_search->setStatusTip("日志搜索");
    connect(log_search, &QAction::triggered, this, &Nav::logSearch);

    go_to_class = new QAction(QIcon("/mySoftware/C++Qt/myProject/NAV/images/class"),"导航去教室", this);
    go_to_class->setStatusTip("导航去教室");
    connect(go_to_class, &QAction::triggered, this, &Nav::gotoClass);

    qDebug()<<"init action down"<<endl;
}

void Nav::initToolBar() {
...
int Nav::getFirstShuttleTime(qreal t) {
...
int Nav::getFirstBusTime(qreal t) {
...
void Nav::setCurrentToVer(int no) {
...
//直接画出完整的路径
void Nav::drawPath(const QVector<Vertex*> &path, bool currentIn) {
        ...
        if(current->get_priority()==Pr_Distance) {
            ...
            if(is_anting==false) {
                ...
                if(current->get_path()==dij_dis_first.get_dij_path()) {
                    ...
                    //current的velocity和Nav的可能不同，current的速度才是行进的速度
...
void Nav::choose_searchnear(int i) {
    //下标对应到顶点编号
    int no = list_to_no[i];
...
void Nav::choose_search_fzs(int i) {
...
void Nav::choose_fp_begin_fzs(int i) {
    fuzzysearch_res = fuzzysearch_vec[i];
...
//将选中的字符串暂存，由每个lineEdit的不同的槽函数进行更改text
void Nav::choose_fp_end_fzs(int i) {
    fuzzysearch_res = fuzzysearch_vec[i];
...
//空串不能模糊搜索，是任何串的子串
void Nav::fuzzySearch(const QString &str) {
    QVector<int> res;
...
void Nav::search_fzs(const QString &str) {
    //空串直接返回
    if(str.size()==0) {
        search_end = -2;
        return;
...
void Nav::findpath_begin_fzs(const QString &str) {
    //如果输入为当前位置，不用模糊搜索
    if(str=="当前位置") {
        findpath_begin = -1;
        return;
...
void Nav::findpath_end_fzs(const QString &str) {
    if(str.size()==0) {
        findpath_end = -2;
        return;
    }
...
void Nav::antfind_fzs(const QString &str) {
    if(str.size()==0) {
        ant_spot = -2;
        return;
    }
...
//开始导航，初始化当前状态
void Nav::setCurrent(const QVector<Vertex*> &path, qreal dis, qreal time, Priority pr) {
...
bool Nav::inShahe(int spot) {
    return spot>=0 && spot<=SHAHE_MAX_NO;
}

bool Nav::inBenbu(int spot) {
    return spot<=BENBU_MAX_NO && spot>=BENBU_BUS_VER_NO;
}

bool Nav::inBetween(int spot) {
